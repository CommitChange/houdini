...
describe :stripe_card do
  ...
  it 'creates 1 Stripe::Customer' do
    ...
    expect(StripeMockHelper.stripe_helper.instance.customers.count).to eq 1
  end

  it 'has created a single source on the customer' do
    create(:stripe_card_base)

    expect(StripeMockHelper.stripe_helper.instance.customers.first[1][:sources][:total_count]).to eq 1
  end
end