...
RSpec.shared_context :dispute_created_context do 
  include_context :disputes_context do 
    ...
    let(:event_json) do 
      event_json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.created')
...
RSpec.shared_context :dispute_funds_withdrawn_context do 
  include_context :disputes_context do 
    ...
    let(:event_json) do 
      event_json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.funds_withdrawn')
      StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, event_json['data']['object'])
      event_json
    end
...
RSpec.shared_context :dispute_funds_reinstated_context do
  ...
  let(:event_json) do
    event_json =StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.funds_reinstated')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, event_json['data']['object'])
    event_json
  end
  let!(:charge) { force_create(:charge, supporter: supporter, 
...
RSpec.shared_context :dispute_lost_context do
  include_context :disputes_context
  let(:event_json) do
    event_json =StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.closed-lost')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, event_json['data']['object'])
    event_json
  end
  let!(:charge) { force_create(:charge, supporter: supporter, 
...
RSpec.shared_context :dispute_won_context do
  include_context :disputes_context
  let(:event_json) do
    event_json =StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.closed-won')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, event_json['data']['object'])
    event_json
  end
  let!(:charge) { force_create(:charge, supporter: supporter, 
...
RSpec.shared_context :dispute_created_and_withdrawn_at_same_time_context do 
  ...
  let(:event_json_created) do
    StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.created-with-one-withdrawn')
  end
  ...
  let(:event_json_funds_withdrawn) do
    json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.funds_withdrawn')
...
RSpec.shared_context :dispute_created_and_withdrawn_in_order_context do 
  include_context :dispute_created_and_withdrawn_at_same_time_context
  let(:event_json_created) do
    json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.created')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, json['data']['object'])
    json
  end
...
RSpec.shared_context :dispute_created_withdrawn_and_lost_in_order_context do 
  ...
  let(:event_json_lost) do
    json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.closed-lost')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, json['data']['object'])
    json
  end
...
RSpec.shared_context :dispute_created_with_withdrawn_and_lost_in_order_context do
  ...
  let(:event_json_created) do
    json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.created-with-one-withdrawn')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, json['data']['object'])
    json
  end
end
...
RSpec.shared_context :__dispute_with_two_partial_disputes_withdrawn_at_same_time_context do
  ...
  let(:event_json_dispute_partial1) do
    json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.created-with-one-withdrawn--partial1')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, json['data']['object'])
    json
  end
  ...
  let(:event_json_dispute_partial2) do
    json = StripeMockHelper.stripe_helper.mock_webhook_event('charge.dispute.created-with-one-withdrawn--partial2')
    StripeMockHelper.stripe_helper.upsert_stripe_object(:dispute, json['data']['object'])
    json
  end
