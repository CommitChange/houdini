...
RSpec.describe InlineJob::ModernObjectDonationStripeChargeJob, type: :job do
  ...
  let(:recent_charge) { force_create(:charge, donation: donation, card: card, amount: 300, status: "paid", created_at: 1.day.ago, payment: recent_payment) }

  let(:recent_payment) { force_create(:payment, gross_amount: 300, date: 1.day.ago, supporter: supporter) }
  let(:performed_job) {
