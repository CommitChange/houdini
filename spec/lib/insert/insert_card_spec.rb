...
describe InsertCard do
  ...
  let(:stripe_card_token) { StripeMockHelper.stripe_helper.generate_card_token(last4: '9191', exp_year:2011)}
  ...
  describe 'for nonprofits' do
    ...
    describe 'card exists' do
      before(:each) {
        @first_card_tok = StripeMockHelper.stripe_helper.generate_card_token(:last4 => '9999', :exp_year => '2122')
    ...
    it 'handle card errors' do
      StripeMockHelper.stripe_helper.prepare_error(
    ...
    it 'handle stripe errors' do
      StripeMockHelper.stripe_helper.prepare_error(Stripe::StripeError.new('card error'), :new_customer)
      card_data = { :holder_type => 'Nonprofit', :holder_id => nonprofit.id, :stripe_card_id => 'card_88888', :stripe_card_token => stripe_card_token, :name => "card_name" }
