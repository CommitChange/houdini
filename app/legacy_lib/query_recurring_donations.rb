...
module QueryRecurringDonations
  ...
  def self.full_search_expr(np_id, query)
    ...
    if query.key?(:active)
      clause = query[:active] ? [is_active_clause('recurring_donations'), is_not_completed_clause('recurring_donations')].join(" AND ") : [is_cancelled_clause('recurring_donations'), is_not_completed_clause('recurring_donations')].join(" AND ")
      failed_or_active_clauses.push("(#{clause})")
    end
    ...
    if query.key?(:completed)
      clause = query[:completed] ? [is_completed_clause('recurring_donations'), is_active_clause('recurring_donations')].join(" AND ") : is_not_completed_clause('recurring_donations')
      failed_or_active_clauses.push("(#{clause})")
    end
  ...
  def self.get_chunk_of_export(npo_id, query, offset=nil, limit=nil, skip_header=false )
    ...
    WHEN #{is_failed_clause('recurring_donations')}
      THEN 'failed'
    WHEN #{is_completed_clause('recurring_donations')}
      THEN 'completed'
    ELSE 'active' END AS status",
    'recurring_donations.cancelled_at AS "Cancelled At"',
    "CASE WHEN (#{is_active_clause('recurring_donations')} OR #{is_failed_clause('recurring_donations')}) AND #{is_not_completed_clause('recurring_donations')} THEN concat('#{root_url}recurring_donations/', recurring_donations.id, '/edit?t=', recurring_donations.edit_token) ELSE '' END AS \"Donation Management Url\"",
  ...
  def self.is_external_active_clause(field_for_rd)
    "#{is_active_clause(field_for_rd)} AND #{is_not_failed_clause(field_for_rd)} AND #{is_not_completed_clause(field_for_rd)}"
  end

  def self.is_external_cancelled_clause(field_for_rd)
    "(#{is_cancelled_clause(field_for_rd)}) AND #{is_not_failed_clause(field_for_rd)} AND (#{is_not_completed_clause(field_for_rd)})"
  end
  ...
  def self.is_completed_clause(field_for_rd)
    "#{field_for_rd}.end_date IS NOT NULL AND #{field_for_rd}.end_date < '#{Time.current}'"
  end

  def self.is_not_completed_clause(field_for_rd)
    "NOT(#{is_completed_clause(field_for_rd)})"
  end
