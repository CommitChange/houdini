...
class FeeStructure < ActiveRecord::Base
  ...
  # @option opts [Integer] :flat_platform_fee (0) the flat platform fee to add to the given fee structure
  ...
  class FeeCalculation
    ...
    attr_accessor :source, :platform_fee, :flat_platform_fee, :amount, :fee_structure
    ...
    def initialize(**args)
      ...
      @source = args[:source]
      @flat_platform_fee = args[:flat_platform_fee] || 0
      @amount = args[:amount]
    ...
    def calculate
      ...
      (BigDecimal.new(amount) * fee_surcharge).ceil + fee_structure.flat_fee + flat_platform_fee
    end
